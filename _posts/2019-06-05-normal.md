...
A random variable $X$ has the **standard normal distribution** if its density function $\varphi\colon\mathbb R\to\mathbb R$ is given as
...
as [can be seen here](https://newonlinecourses.science.psu.edu/stat414/node/166/). Now, let's see a few examples of normally distributed variables, with the associated python code.
...
If we have given an infinite sequence $X_1, X_2, \dots$ of random variables then we define the **n'th sample mean** to be

$$ \overline X_n := \frac{X_1 + X_2 + \dots + X_n}{n}, $$

i.e. when we're given a data set containing $n$ samples then this is precisely the usual mean that we would be calculating. The difference is of course that here we're talking about *random* variables, so in a data set we'd be working with a particular *instance* of these variables. Working in the more general set up however, we can prove some interesting facts about the sample means.
...
Here we used that [expectation is linear](https://brilliant.org/wiki/linearity-of-expectation/). A stronger result is true in fact, as the following theorem shows that the sample means *converge* to the true mean:
...
Now, we say that we **standardise** a random variable when we subtract the mean and divide by the standard deviation: this is precisely what Scikit-Learn's `StandardScaler` is doing as well. The following theorem is justifying why this seems like a good idea:
...
In slightly more layman's terms: *when we standardise our observations then we approximate a standard normal distribution*. The more samples we have, the closer the distribution will look like a standard bell curve. Hoorah!
